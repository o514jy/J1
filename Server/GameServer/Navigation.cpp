...
Navigation::Navigation(RoomBaseRef room) :
	m_geom(0),
	m_navMesh(0),
	m_navQuery(0),
	m_crowd(0),
...
void Navigation::UpdateTick()
{
	if (m_crowd != nullptr)
	{
		m_crowd->update(TICK_COUNT_SEC, nullptr);

		// 순회하면서 위치 업데이트
		int arrSize = m_crowd->getAgentCount();
		for (int i = 0; i < arrSize; i++)
		{
			dtCrowdAgent* agent = m_crowd->getEditableAgent(i);
			if (agent == nullptr)
				continue;
...
void Navigation::handleMeshChanged(InputGeom* geom)
{
	m_geom = geom;
...
bool Navigation::handleBuild()
{
	...
	// init crowd
	m_crowd->init(MAX_AGENTS, getAgentRadius(), m_navMesh);
